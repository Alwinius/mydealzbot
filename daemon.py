...
def send_or_edit(bot, update, text, reply_markup=None):
    try:
        ...
        try:
            bot.editMessageText(text=text, chat_id=chat_id, message_id=message_id, reply_markup=reply_markup, parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True)
        except Unauthorized:
            session = DBSession()
        ...
        bot.sendMessage(text=text, chat_id=update.message.chat.id, reply_markup=reply_markup, parse_mode=telegram.ParseMode.MARKDOWN, disable_web_page_preview=True)
...
def CheckUser(bot, update):
    ...
    if not entry:
        # Nutzer ist neu
        new_user = User(id=chat.id, first_name=chat.first_name, last_name=chat.last_name, username=chat.username, title=chat.title, counter=0, current_selection="0")
...
def ShowAlert(bot, update, keyword):
    ...
    button_list = [[InlineKeyboardButton("💣 Benachrichtigung löschen", callback_data="4$" + str(keyword.id) + "$0"), InlineKeyboardButton("💶 Maximalpreis bearbeiten", callback_data="3$" + str(keyword.id))],
    [InlineKeyboardButton("🗃️ Kategorie ändern", callback_data="5$" + str(keyword.id)), InlineKeyboardButton("🔍 Suchbegriffe bearbeiten", callback_data="2$" + str(keyword.id))],
    [InlineKeyboardButton("🏷️ Reichweite ändern", callback_data="6$" + str(keyword.id)), InlineKeyboardButton("🏠 Home", callback_data="0")]]
...
def SetCategory(bot, update, keyword, value=None):
    categories = ["Alle", "Fashion & Accessoires", "Elektronik", "Entertainment", "Home & Living", "Dies & Das", "Reisen", "Food", "Apps & Software", "TV & HiFi", "Verträge und Finanzen", "Beauty", "Coupons", "Handy & Tablets", "Freebies", "Family & Kids"]
    ...
    if value is None:
        ...
        while counter < len(categories):
            button_list.append([InlineKeyboardButton(categories[counter], callback_data="5$" + str(keyword.id) + "$" + categories[counter]), InlineKeyboardButton(categories[counter + 1], callback_data="5$" + str(keyword.id) + "$" + categories[counter + 1])])
            counter += 2
        send_or_edit(bot, update, message, InlineKeyboardMarkup(button_list))
    else:
        if value in categories:
            if not keyword:
                send_or_edit(bot, update, "Die Benachrichtigung kann nicht gefunden werden", InlineKeyboardMarkup(button_list))
            else:
                keyword.category = value
                button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("📜 Übersicht", callback_data="1$" + str(keyword.id)), InlineKeyboardButton("💶 Maximalpreis ändern", callback_data="3$" + str(keyword.id))]]
...
def SetScope(bot, update, keyword, value=None):
    ...
    if value is None:
        ...
        button_list = [[InlineKeyboardButton("Nur Titel", callback_data="6$" + str(keyword.id) + "$0"), InlineKeyboardButton("Titel & Beschreibung", callback_data="6$" + str(keyword.id) + "$1")]]
        send_or_edit(bot, update, message, InlineKeyboardMarkup(button_list))
    else:
        if value in ["0", "1"]:
            if not keyword:
                ...
                keyword.scope = value
                button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("📜 Übersicht", callback_data="1$" + str(keyword.id))]]
...
def SaveMaxPrice(bot, update, usr, alertid):
    ...
    if re.match(r"^[0-9]+(,[0-9]{0,2})?$", update.message.text) is not None and len(update.message.text) < 10:
        ...
        if not keyword:
            send_or_edit(bot, update, "Die Benachrichtigung kann nicht gefunden werden", InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Home", callback_data="0")]]))
        else:
            keyword.maxprice = maxprice
            s.commit()
            button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("📜 Übersicht", callback_data="1$" + str(alertid)), InlineKeyboardButton("🏷️ Reichweite ändern", callback_data="6$" + str(alertid))]]
            send_or_edit(bot, update, "Maximalpreis aktualisiert.", InlineKeyboardMarkup(button_list))
        s.close()
    else:
        send_or_edit(bot, update, "Bitte einen gültigen Preis als ganze Zahl oder mit zwei Nachkommastellen angeben.", None)
...
def SaveKeywords(bot, update, usr, alertid=None):
    ...
    for keyw in keywords:
        ...
        if re.search(r"^[a-zA-Z0-9äöüÄÖÜß .-]+$", keyw) is None:
            send_or_edit(bot, update, "Der Ausdruck enthält ungültige Zeichen oder entspricht nicht der vorgeschriebenen Formatierung. Es sind nur Buchstaben und die Sonderzeichen .- erlaubt. Suchbegriffe oder Wortgruppen müssen durch ein Komma getrennt werden.")
            return 
    ...
    if alertid is not None:
        ...
        if not keyword:
            ...
            button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("📜 Übersicht", callback_data="1$" + str(alertid)), InlineKeyboardButton("🗃️ Kategorie ändern", callback_data="5$" + str(alertid))]]
        ...
        s.add(new_alert)
        s.commit()
        button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("📜 Übersicht", callback_data="1$" + str(new_alert.id)), InlineKeyboardButton("🗃️ Kategorie ändern", callback_data="5$" + str(new_alert.id))]]
        send_or_edit(bot, update, "Die Suchbegriffe wurden gespeichert.", InlineKeyboardMarkup(button_list)) 
    s.close()
...
def DeleteAlert(bot, update, conf, keyword, s):
    if conf == "1":
        s.delete(keyword)
        send_or_edit(bot, update, "Der Suchauftrag '" + keyword.keywords + "' wurde erfolgreich gelöscht.", InlineKeyboardMarkup([[InlineKeyboardButton("🏠 Home", callback_data="0")]]))
    else:
        button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("💣 Löschen", callback_data="4$" + str(keyword.id) + "$1")]]
        send_or_edit(bot, update, "Möchtest du den Suchauftrag '" + keyword.keywords + "' wirklich löschen?", InlineKeyboardMarkup(button_list))    
    ...
    if len(args) > 1: #check the alert here
        ...
        if not keyword:
            ...
            if args[0] == "1": #show current information about Alert
                ...
                usr = CheckUser(bot, update)
                send_or_edit(bot, update, "Bitte neuen Maximalpreis angeben. \"0,00\" deaktiviert den Maximalpreisfilter.", None)
            elif int(args[0]) == 4 and len(args) > 2: # Alert löschen
                DeleteAlert(bot, update, args[2], keyword, s)
            elif int(args[0]) == 2:
                send_or_edit(bot, update, "Bitte gib eine Liste durch Komma getrennter Suchbegriffe oder Wortgruppen an. Der Alarm wird ausgelöst sobald ein Suchbegriff oder eine Wortgruppe gefunden wird. Die Groß-/ Kleinschreibung wird dabei nicht beachtet.", None)
            else:
